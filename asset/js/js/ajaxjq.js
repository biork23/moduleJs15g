...
$(document).ready(function(){
    ...
    $.ajax({
        method: 'GET',
        url: 'https://ismael-15gjs-default-rtdb.firebaseio.com/posts/.json'
    }).done( (data) => {
        console.log(data)
        let temp = ''
        for(post in data) {
    ...
    $.ajax({
        method:'PATCH',
        url:`https://ismael-15gjs-default-rtdb.firebaseio.com/posts/${idPostUpdate}.json`,
        data: JSON.stringify(
            {
    ...
    const deletePost = () => {
        ...
        $.ajax({
            method:'DELETE',
            url: `https://ismael-15gjs-default-rtdb.firebaseio.com/posts/${idPostDelete}.json`
        }).done(function(response){
            console.log(response)
            // do something
        }) .fail( function (err) {
